...
완화: **`MaxParallelActions` 파라미터** (Build Editor) — 동시 실행 액션 수를 낮춰 피크 메모리를 RAM 아래로. 빌드 스텝이 에이전트 `%PROGRAMDATA%\...\BuildConfiguration.xml`에 **XML 머지**(Horde 등 보존)로 주입하므로 **엔진 소스/머신 파일 수동편집 불필요**. 기본 10, OOM/속도 보며 튜닝.
- 단, `MaxParallelActions`는 컴파일·링크 통틀어 제한 → 낮추면 원격 컴파일 처리량도 ↓ (속도↓/안정성↑ 트레이드오프).
- 근본 해법: RAM 증설(64GB+) 또는 DebugGame 제거(엔진 소스 수정). 페이지파일은 UBA가 *물리 RAM* 기준 kill이라 효과 없음.
...
- 2026-06 (4차): `MaxParallelActions` 파라미터 추가 — Build Editor 스텝이 에이전트 BuildConfiguration.xml에 XML 머지로 주입(Horde 보존, 엔진 소스 무관). Link 메모리 OOM→UBA 크래시 완화(함정 #14).
